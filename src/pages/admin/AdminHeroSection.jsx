...
import { Sparkles, Save, RotateCcw, Plus, Trash2, ShieldCheck } from 'lucide-react';
  ...
  return (
    <div className="space-y-5">
      <div className="border-b border-slate-200 pb-3">
        ...
        <p className="text-xs text-slate-500 mt-0.5">Customize the landing page surgical headline, robotic background visual, and key clinical counters.</p>
      </div>

      <form onSubmit={handleSave} className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 sm:p-8 space-y-4 text-xs">
        {[
          { key: 'badge', label: 'Top Badge Label', placeholder: 'Center of Excellence in Robotic Joint Replacement' },
          { key: 'heading', label: 'Main Headline', placeholder: 'Pioneering Sub-Millimeter Robotic Joint Replacement & Pain-Free Spine Surgery' },
          { key: 'heroImage', label: 'Robotic Surgery Background Image URL', placeholder: 'https://images.unsplash.com/...' },
          { key: 'emergencyLabel', label: 'Emergency Card Title', placeholder: '24/7 Level-1 Fracture & Trauma Helpline' },
          { key: 'emergencyHotline', label: 'Emergency Helpline Number', placeholder: '1800-419-6784' },
        ].map((f) => (
          <div key={f.key}>
        ...
        <div>
          <label className="block font-bold text-slate-700 mb-1">Subheadline & Surgical Description</label>
          <textarea
            rows={3}
        ...
        <div className="pt-2 border-t border-slate-100">
          <div className="flex items-center justify-between mb-2">
            <label className="font-bold text-slate-700 uppercase tracking-wider text-[11px]">Surgical Performance Counters</label>
            <button
              type="button"
              onClick={addStat}
              className="px-3 py-1 text-xs font-bold text-blue-700 bg-blue-50 hover:bg-blue-100 border border-blue-200 rounded-xl flex items-center space-x-1 cursor-pointer"
            >
            ...
            {(form.stats || []).map((stat, idx) => (
              <div key={idx} className="flex items-center space-x-2">
                <input
                  ...
                  placeholder="12,000+"
                  className="w-36 px-3 py-2 bg-slate-50 border border-slate-300 rounded-xl text-xs focus:outline-none focus:border-blue-600 font-mono font-bold"
                />
                <input
                  type="text"
                  value={stat.label}
                  onChange={(e) => updateStat(idx, 'label', e.target.value)}
                  placeholder="Robotic Surgeries"
                  className="flex-1 px-3 py-2 bg-slate-50 border border-slate-300 rounded-xl text-xs focus:outline-none focus:border-blue-600 font-semibold"
                />
                <button
                  type="button"
