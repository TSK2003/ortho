...
const BlogDetailPage = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-16 space-y-10">
      <PageHero
        title={post.title}
        subtitle={`Written by ${post.author} • Orthopedic Patient Care Guide`}
        breadcrumb={[{ label: 'Patient Guides', path: '/blog' }, { label: post.title }]}
      />
        ...
        <div className="bg-white rounded-3xl p-6 sm:p-10 shadow-sm border border-slate-200 space-y-6">
          <div className="flex flex-wrap items-center justify-between gap-4 text-xs text-slate-500 border-b border-slate-100 pb-4">
            <div className="flex items-center space-x-3">
              <span className="px-3 py-1 bg-blue-50 text-blue-800 font-bold rounded-lg">{post.category}</span>
          ...
          <div className="text-slate-700 text-xs sm:text-sm leading-relaxed space-y-4">
            ...
            {typeof post.content === 'string' && post.content.includes('<') ? (
              <div 
                className="prose prose-blue max-w-none text-slate-600 leading-relaxed space-y-3"
          ...
          <div className="pt-6 border-t border-slate-100 flex justify-between items-center">
            <Link to="/blog" className="text-xs font-bold text-blue-700 hover:text-blue-900 flex items-center space-x-1">
              <ArrowLeft className="w-3.5 h-3.5" />
              <span>Back to Patient Guides</span>
            </Link>
          </div>
        </div>
      </div>
